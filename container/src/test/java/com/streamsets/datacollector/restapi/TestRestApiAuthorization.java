...
public class TestRestApiAuthorization {
  ...
  private String startServer(boolean authzEnabled) throws  Exception {
    ...
    Writer writer = new FileWriter(new File(System.getProperty(RuntimeModule.SDC_PROPERTY_PREFIX + RuntimeInfo.CONFIG_DIR), "coraline.properties"));
    conf.save(writer);
    writer.close();
  ...
  private List<RestApi> createRestApis() throws Exception {
    ...
    list.add(new RestApi("/rest/v1/system/stats", Method.GET, AuthzRole.ALL_ROLES));
    list.add(new RestApi("/rest/v1/system/stats", Method.POST, AuthzRole.ADMIN));
