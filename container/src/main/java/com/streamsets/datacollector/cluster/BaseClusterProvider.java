...
public abstract class BaseClusterProvider implements ClusterProvider {
  ...
  // Comma separated list of cor.properties configs that should not be passed from master sdc to slave sdcs
  ...
  private ApplicationState startPipelineInternal(
    ...
    try {
      TarFileCreator.createLibsTarGz(
          ...
          findJars("aster-client", asterClientCL, null),
          streamsetsLibsCl,
          userLibsCL,
          staticWebDir,
          libsTarGz
    ...
    try {
      ...
      sdcPropertiesFile = new File(etcDir, "cor.properties");
      if (executionMode == ExecutionMode.CLUSTER_MESOS_STREAMING) {
  ...
  void copyDpmTokenIfRequired(Properties sdcProps, File etcStagingDir) throws IOException {
    ...
    } else { //config.includes won't be there for parcels installation, all configs in cor.properties
