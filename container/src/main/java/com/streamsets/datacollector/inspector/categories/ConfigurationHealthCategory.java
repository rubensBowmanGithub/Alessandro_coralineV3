...
public class ConfigurationHealthCategory implements HealthCategory {
  ...
  public HealthCategoryResult inspectHealth(Context context) {
    ...
    builder.addHealthCheck("Max Batch Size for Preview", HealthCheck.Severity.smallerIsBetter(maxBatchSizeForPreview, 2_000, 5_000))
        ...
        .withDescription("Maximal number of records that can be created in a single batch while in preview. Controlled by {} in cor.properties", PreviewResource.MAX_BATCH_SIZE_KEY);
    ...
    builder.addHealthCheck("Max Batch Size", HealthCheck.Severity.smallerIsBetter(maxBatchSize, 50_000, 100_000))
        ...
        .withDescription("Maximal number of records that origin should create in a single batch while running a pipeline. Controlled by {} in cor.properties", Constants.MAX_BATCH_SIZE_KEY);
    ...
    builder.addHealthCheck("Max Error Records Per Stage", HealthCheck.Severity.smallerIsBetter(maxErrorRecordsPerStage, 101, 1_000))
        ...
        .withDescription("Maximal number of error records that should be kept in memory while pipeline is running per each stage of each running pipeline. Controlled by {} in cor.properties", Constants.MAX_ERROR_RECORDS_PER_STAGE_KEY);
    ...
    builder.addHealthCheck("Max Pipeline Errors", HealthCheck.Severity.smallerIsBetter(maxPipelineErrors, 101, 1_000))
        ...
        .withDescription("Maximal number of errors that should be kept in memory while pipeline is running per running each pipeline. Controlled by {} in cor.properties", Constants.MAX_PIPELINE_ERRORS_KEY);
    ...
    builder.addHealthCheck("Max Log Tailers", HealthCheck.Severity.smallerIsBetter(maxLogTailers, 6, 20))
        ...
        .withDescription("Maximal number of concurrent sessions that can be tailing logs in Data Collector Logs view. Controlled by {} in cor.properties", LogMessageWebSocket.MAX_LOGTAIL_CONCURRENT_REQUESTS_KEY);
    ...
    builder.addHealthCheck("Max Private ClassLoader", HealthCheck.Severity.smallerIsBetter(maxPrivateStageLoaders, 51, 150))
        .withValue(maxLogTailers)
        .withDescription("Maximal number of stages that requires separate instance of a classloader that can be running at a time. Controlled by {} in cor.properties", ClassLoaderStageLibraryTask.MAX_PRIVATE_STAGE_CLASS_LOADERS_KEY);
    ...
    builder.addHealthCheck("Max Runner Size", HealthCheck.Severity.smallerIsBetter(maxRunnerSize, 100, 500))
        ...
        .withDescription("Maximal size of thread pool that is responsible for running pipelines. Controlled by {} in cor.properties", ExecutorConstants.RUNNER_THREAD_POOL_SIZE_KEY);
    ...
    builder.addHealthCheck("Max Pipeline Runner Size", HealthCheck.Severity.smallerIsBetter(maxPipelineRunnerSize, 51, 100))
        ...
        .withDescription("Maximal number of Pipeline Runners a single multi threaded pipeline can create. Controlled by {} in cor.properties", Pipeline.MAX_RUNNERS_CONFIG_KEY);
