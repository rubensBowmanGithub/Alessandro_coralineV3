...
public class RuntimeEL {
  ...
  private static final String SDC_PROPERTIES = "cor.properties";
  ...
  public static void loadRuntimeConfiguration(RuntimeInfo runtimeInfo) throws IOException {
    ...
    The cor.properties file has a property 'runtime.conf.location' with possible values - 'embedded' or 'properties
    file name'.
    If the value is 'embedded', the runtime configuration properties will be read from the cor.properties,
    ...
    if (configFile.exists()) {
      ...
      if(runtimeConfLocation.equals(RUNTIME_CONF_LOCATION_DEFAULT)) {
        //runtime configuration is embedded in cor.properties file. Find, trim, cache.
