...
public class TestBootstrapMain {
  ...
  public void testGetStageLibrariesClasspaths() throws Exception {
    String baseDir = getBaseDir();
    String stageLibsDir = baseDir + BootstrapMain.FILE_SEPARATOR + "coraline-libs";
    ...
    Assert.assertNotNull(String.valueOf(libs.keySet()), libs.get("coraline-libs/stage1"));
    Assert.assertNotNull(String.valueOf(libs.keySet()), libs.get("coraline-libs/stage2"));
    Assert.assertEquals(String.valueOf(libs.get("coraline-libs/stage1")), 3,
      libs.get("coraline-libs/stage1").size());
    Assert.assertEquals(String.valueOf(libs.get("coraline-libs/stage2")), 2,
      libs.get("coraline-libs/stage2").size());
  }
  ...
  public void testMainInvocation() throws Exception {
    ...
    String streamsetsLibsDir = baseDir + BootstrapMain.FILE_SEPARATOR + "coraline-libs";
    ...
    // add this to properties - create coraline.properties file.
    props.setProperty(PARSER_LIMIT, PARSER_LIMIT_VALUE);
    try (OutputStream os = new FileOutputStream(new File(dir, "coraline.properties"))) {
      props.store(os, "");
    }
